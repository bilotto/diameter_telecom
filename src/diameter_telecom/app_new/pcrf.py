from diameter_telecom.session._diameter_session import DiameterSession
...
class PcrfGxApplication(CommonThreadingApplication):
    ...
    def _handle_request(self, message: CreditControlRequest) -> CreditControlAnswer:
        ...
        if self.session_manager.sessions.get_session_by_id(APP_3GPP_GX, message.session_id):
            gx_session: GxSession = self.session_manager.sessions.get_session_by_id(APP_3GPP_GX, message.session_id)
        else:
        ...
        if message.cc_request_type == E_CC_REQUEST_TYPE_INITIAL_REQUEST:
            ...
            # Get apn
            apn = gx_session.called_station_id
            if not apn:
                self.logger.error(f"{__class__.__name__}❌ Gx CCR: No APN found for session {message.session_id}")
                pass
            ...
            if message.rat_type and message.rat_type == E_RAT_TYPE_EUTRAN:
                ...
                answer.charging_rule_install.append(ChargingRuleInstall("UNKNOWN_RAT_TYPE"))
                self.logger.error(f"{__class__.__name__}❌ Gx CCR: Unknown RAT type {message.rat_type} for session {message.session_id}")
            ...
            if self.rx_app:
                ...
                answer.charging_rule_install.append(ChargingRuleInstall("VOICE_RULE"))
                pass
...
def identify_voice_call(request: AaRequest) -> bool:
    return True
...
class PcrfRxApplication(CommonThreadingApplication):
    ...
    def _handle_request(self, message: Message):
        ...
        if isinstance(message, AaRequest):
            ...
            if not rx_session:
                self.logger.error(f"{__class__.__name__}❌ Rx AAR: Rx session {message.session_id} not found for voice call")
                raise ValueError(f"Rx session {message.session_id} not found")
            gx_session: DiameterSession | None = self.session_manager.sessions.get_session_by_id(APP_3GPP_GX, rx_session.gx_session_id)
            if not gx_session:
                self.logger.error(f"{__class__.__name__}❌ Rx AAR: Gx session {rx_session.gx_session_id} not found for voice call")
