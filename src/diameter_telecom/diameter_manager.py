...
class DiameterManager:
    ...
    def start(self):
        startup_order = define_node_startup_order(list(self._nodes.values()))
    ...
    def create_application_service(self, applications: List[CommonThreadingApplication], diameter_config: dict) -> ApplicationService:
        application_service = ApplicationService(applications, diameter_config, session_manager=self.session_manager, subscribers=self.subscribers)
